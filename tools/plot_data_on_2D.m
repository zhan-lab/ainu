...
clear;
datafilename = 'ainu_result_sim1_trpl_22_1960_231024_18_asEnmmBC_';
...
function scathist_pdf(mat_file,x_ind,y_ind,visible_flag,save_path)
    ...
    if x_top_s >= 0.9*x_top
        ...
        elseif dx_max>0.1 && dx_min>0.1
            ...
            x_txt=[sprintf('%.2g',x_2v(1)),'\pm',sprintf('%.2g',x_top_s)];
    ...
    if y_top_s >= 0.9*y_top
        ...
        elseif dy_max>0.1 && dy_min>0.1
            ...
            y_txt=[sprintf('%.2g',y_2v(1)),'\pm',sprintf('%.2g',y_top_s)];
            txt_y_position = 'left';
        end
    else
        y_txt=[sprintf('%.2g',y_2v(1)),'\pm',sprintf('%.2g',y_top_s)];
